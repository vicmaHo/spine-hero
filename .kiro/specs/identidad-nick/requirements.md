...
Para limitar la creación masiva de nicks por una misma persona, la primera alta pide además un **correo electrónico** que queda vinculado al nick: un correo ya usado no puede dar de alta un segundo nick. Ese mismo correo se vuelve a pedir al entrar con un nick ya creado, y el acceso solo se concede si el nick pertenece a ese correo: no es una contraseña —el correo no se verifica— pero evita que baste conocer un nick ajeno para entrar con él.
...
Este esquema **no autentica a nadie**: la pareja nick + correo estorba a quien solo conoce un nick, pero el correo se acepta sin verificar su propiedad, la comprobación la resuelve el cliente y las credenciales de invitado permiten leer los modelos donde vive el correo, así que quien se lo proponga puede rodearla. Al no haber propiedad por Cognito, cualquier cliente invitado puede además sobrescribir el `DailyRecord` del día de otro nick. Es una decisión consciente, proporcional a un ranking amistoso de hackathon con datos no sensibles (segundos de buena postura, nivel, XP), y queda documentada como limitación en el Requisito 10 en lugar de disimularse.
...
- **Correo_Vinculado**: Dirección de correo electrónico de 6 a 254 caracteres introducida en el alta de un Nick, normalizada a minúsculas ASCII y sin espacios en los extremos. Tiene dos usos, y ninguno más: limitar el número de Nick por persona y comprobar, al entrar con un Nick ya creado, que ese Nick es de quien lo reclamó.
...
- **Formulario_Acceso**: Componente de `src/ui/` que solicita el Nick y el Correo_Vinculado, en sus dos modos, antes de dar paso al Dashboard.
...
### Requisito 2: Acceso con un nick existente comprobando su correo

**User Story:** Como usuario que ya tiene nick, quiero entrar escribiendo mi nick y el correo con el que lo creé, para que nadie que solo conozca mi nick pueda entrar en mi nombre.

#### Criterios de Aceptación

1. THE Formulario_Acceso SHALL ofrecer dos modos mutuamente excluyentes y seleccionables por el usuario: «Ya tengo nick» y «Crear nick», ambos con los mismos dos campos, Nick y Correo_Vinculado, siendo «Crear nick» el modo activo inicial cuando el Almacen_Local_Identidad no contiene un Nick.
2. WHILE el modo activo es «Ya tengo nick», THE Formulario_Acceso SHALL presentar exactamente dos campos de entrada, el del Nick y el del Correo_Vinculado, sin campo de contraseña.
3. WHEN el usuario envía en modo «Ya tengo nick» un Nick que cumple el patrón del Requisito 1 criterio 2 y un Correo_Vinculado que cumple el patrón del Requisito 1 criterio 7, y existe un registro UserIdentity cuyo `email` coincide con el Correo_Vinculado normalizado enviado Y cuyo Nick_Normalizado coincide con el del Nick enviado, THE Sistema_Identidad SHALL conceder el acceso al Dashboard en un plazo máximo de 3 segundos desde el envío, SHALL adoptar como identidad activa el Nick tal como está almacenado en ese registro (no como lo escribió el usuario) y SHALL persistir la identidad según el Requisito 4.
4. IF el usuario envía en modo «Ya tengo nick» una pareja (Nick, Correo_Vinculado) que no coincide con ningún registro UserIdentity, THEN THE Formulario_Acceso SHALL mostrar el mensaje «Ese nick y ese correo no coinciden. Comprueba los dos e inténtalo de nuevo» en un plazo máximo de 3 segundos desde el envío, SHALL mantener el modo «Ya tengo nick» activo sin crear ningún registro UserIdentity ni escribir en el Almacen_Local_Identidad, y SHALL ofrecer un control que cambie al modo «Crear nick» conservando el Nick introducido.
5. THE Sistema_Identidad SHALL conceder el acceso en modo «Ya tengo nick» basándose exclusivamente en la coincidencia de la pareja (Nick_Normalizado, Correo_Vinculado normalizado) con un registro UserIdentity, sin solicitar ni comprobar contraseña, código de verificación, propiedad del correo ni ningún otro factor de autenticación (limitación declarada en el Requisito 10).
6. WHEN el usuario alterna entre los modos «Ya tengo nick» y «Crear nick», THE Formulario_Acceso SHALL conservar el Nick y el Correo_Vinculado ya introducidos en sus campos y SHALL retirar los mensajes de error generados en el modo anterior.
...
8. IF la consulta de titularidad en modo «Ya tengo nick» falla o no obtiene respuesta en 10 segundos, THEN THE Sistema_Identidad SHALL abandonar el intento sin conceder el acceso al Dashboard y sin escribir en el Almacen_Local_Identidad, y THE Formulario_Acceso SHALL informar del fallo según el Requisito 8 criterio 7 conservando el Nick y el Correo_Vinculado introducidos.
9. THE Formulario_Acceso SHALL mostrar el mismo mensaje del criterio 4, sin ninguna diferencia de texto, de campo señalado ni de tiempo de respuesta perceptible, tanto cuando el Correo_Vinculado enviado no figura en ningún registro UserIdentity como cuando figura en un registro cuyo Nick es otro, de modo que el mensaje no permita averiguar si una dirección de correo está registrada.
10. IF el usuario envía en modo «Ya tengo nick» un Correo_Vinculado que no cumple el patrón del Requisito 1 criterio 7, THEN THE Formulario_Acceso SHALL mostrar el mensaje «Introduce un correo electrónico válido» y SHALL no realizar ninguna consulta al Sistema_Data.
11. THE Sistema_Identidad SHALL resolver la comprobación del criterio 3 con una única consulta indexada por `email` cuya selección de campos excluya `email`, sin consultar por Nick_Normalizado, de modo que el correo almacenado de una identidad no llegue nunca al navegador y no baste conocer un Nick para descubrir el correo de su titular.
...
2. WHILE el modo activo es «Crear nick», THE Formulario_Acceso SHALL mostrar junto al campo de correo, visible sin ninguna interacción previa del usuario, el texto explicativo «Tu correo solo se usa para evitar nicks duplicados. En el ranking solo aparece tu nick»; y WHILE el modo activo es «Ya tengo nick», SHALL mostrar en ese mismo sitio y con la misma visibilidad un texto que indique que se pide el correo con el que se creó el Nick y que sirve para comprobar que el Nick es de quien entra.
...
5. WHILE el correo introducido está vacío o no cumple el patrón del Requisito 1 criterio 7, THE Formulario_Acceso SHALL presentar el botón de envío deshabilitado en cualquiera de los dos modos, porque los dos exigen el Correo_Vinculado (Requisito 2 criterio 2).
...
1. THE Sistema_Identidad SHALL transmitir el Correo_Vinculado al Sistema_Data exclusivamente en tres operaciones: la consulta de existencia de Correo_Vinculado y la creación del registro UserIdentity, ambas del alta, y la consulta de titularidad del acceso con Nick existente (Requisito 2 criterio 11); sin incluirlo en el cambio de Nick (Requisito 5), en la escritura de DailyRecord ni en ninguna otra consulta o suscripción. En las tres, el Correo_Vinculado es la clave de la operación y su valor lo acaba de escribir la persona que lo envía: ninguna respuesta del Sistema_Data devuelve el `email` almacenado de una identidad.
...
1. THE documento `docs/PRIVACY.md` SHALL declarar, en la sección del criterio 7, que el acceso exige la pareja (Nick, Correo_Vinculado) pero no comprueba la propiedad del correo, de modo que quien conozca las dos cosas puede obtener acceso con ese Nick y escribir DailyRecord con ese `displayName`, y que conocer solo el Nick ya no basta.
...
10. THE documento `docs/PRIVACY.md` SHALL declarar, en la sección del criterio 7, que la comprobación de titularidad del Requisito 2 criterio 3 la resuelve el cliente y descansa en que el `email` almacenado no se pueda leer, mientras que las Credenciales_Invitado del Requisito 6 criterio 6 autorizan la lectura de los modelos UserIdentity y EmailClaim: un cliente construido a mano puede pedir el campo `email` en la selección o listar EmailClaim y recuperar así el correo asociado a un Nick, con lo que la comprobación deja de suponer un obstáculo para quien lo haga. THE documento SHALL indicar que cerrarlo requiere mover la comprobación a una función del lado servidor o restringir la lectura de esos dos modelos, y que no se ha hecho en esta versión.
